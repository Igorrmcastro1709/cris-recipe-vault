            ...
            <div className="space-y-4">
              {pending.map((r) => (
                <div key={r.id} className="bg-card border border-border/60 rounded-2xl p-5 flex flex-col md:flex-row gap-5">
                  {r.image && <img src={r.image} alt={r.title} className="w-full md:w-40 h-32 object-cover rounded-xl" />}
                  <div className="flex-1">
                    ...
                    {r.ingredients.length > 0 && (
                      <p className="text-sm text-muted-foreground mt-2">
                        <strong className="text-foreground">Ingredientes:</strong> {r.ingredients.slice(0, 4).join(", ")}…
                      </p>
                    )}
                    <p className="text-sm text-muted-foreground mt-1">
                      <strong className="text-foreground">Passos:</strong> {r.steps.length} etapas
                    </p>
                    <a href={r.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary text-sm font-medium mt-3 hover:underline">
