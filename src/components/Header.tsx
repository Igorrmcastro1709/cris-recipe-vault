  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur bg-background/80 border-b border-border/60">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
        <Link to="/" className="font-serif text-xl font-bold text-foreground tracking-tight shrink-0">
          Receitas da Cris
        </Link>
          ...
          {!loading && (
            user ? (
              <div className="flex items-center gap-3 pl-3 border-l border-border/60">
                <span className="hidden sm:inline-flex items-center gap-1.5 text-xs text-muted-foreground" title={user.email ?? ""}>
                  {isAdmin && <ShieldCheck size={13} className="text-primary" aria-label="Administradora" />}
                  {user.email?.split("@")[0]}
                </span>
                <button
                  onClick={() => signOut()}
              ...
              <Link to="/login" className="inline-flex items-center gap-1.5 bg-primary text-primary-foreground px-3 py-1.5 rounded-full text-xs font-semibold hover:opacity-90 transition">
